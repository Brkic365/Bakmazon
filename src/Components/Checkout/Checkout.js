...
function Checkout() {
    ...
    const [shipping, setShipping] = useState(125);
    const [taxes, setTaxes] = useState(15.25);
    ...
    const namesList = [["First Name", "Last Name"], "Address", ["ZIP", "Email Address"], ["Country", "Town/City"], ["Company name (optional)", "Phone (optional)"]];
    let newPrice = 0;
    ...
    useEffect(() => {
        ...
        let promises = Object.keys(cart).map(async (key) => {
            ...
            return new Promise((res, rej) => {res(newPrice)});
        })
