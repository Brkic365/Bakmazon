        ...
        .onSnapshot((snapshot) => {
          if (snapshot.data()) {
            ...
            if (Object.values(snapshot.data()).length > 0) {
              setItemCount(
                Object.values(snapshot.data()).reduce((a, b) => a + b)
              );
            } else {
              setItemCount(0);
            }
    ...
    <div className="navbar">
      <nav>
        ...
        <Link to="/" className="navbar__logo-link">
          <h1 className="navbar__logo">BAKMAZON</h1>
        </Link>
        ...
        >
          <BiExit
            className="navbar__mobile-menu-exit"
        ...
        <div className="navbar__icons">
          {username.length > 1 ? (
            <div
              className="navbar__user-and-dropdown"
              onMouseLeave={() => setDisplayAccDropdown(false)}
            >
              <Link
                className="navbar__user"
                onMouseEnter={() => setDisplayAccDropdown(true)}
                to="/account"
              >
                <p className="navbar__username">
                  {username[0]} <br /> {username[1]}
                </p>
                {pfpUrl ? (
                  <img
                    src={pfpUrl}
              ...
              <motion.div
                ...
                animate={displayAccDropdown ? "open" : "closed"}
            ...
            <div className="navbar__user">
              <Link to="/login" className="navbar__user-link">
                <FaUserPlus className="navbar__user-img navbar__user-img-plus" />
              </Link>
            </div>
          )}
          ...
          <div
            className="navbar__cart-and-dropdown"
            onMouseLeave={() => setDisplayCartDropdown(false)}
          >
            <Link
              to="/cart"
              className="navbar__cart-link"
              onMouseEnter={() => setDisplayCartDropdown(true)}
            >
              <div className="navbar__cart">
                <img src={cart} alt="cart" className="navbar__cart-img" />

                <p
                  className="navbar__cart-num"
                  style={{
                    left: `${32 - 7 * (itemCount.toString().length - 1)}%`,
                    fontSize: `${
                      itemCount.toString().length >= 3 ? "1em" : "1.2em"
                    }`,
                  }}
                >
                  {itemCount}
            ...
            <motion.div
              className="navbar__dropdown"
              animate={displayCartDropdown ? "open" : "closed"}
              variants={dropdownVariants}
            >
              <CartDropdown />
            </motion.div>
