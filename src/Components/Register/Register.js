...
function Register() {
    ...
    const [minCharCountPassword, setMinCharCountPassword] = useState(6);
    ...
    const handleRegister = (e) => {
        if(!nameValid || !emailValid || !passwordValid || !repeatedPasswordValid) {
            setNameErrorMessage(!nameValid ? "First and last name are required!" : "")
            setEmailErrorMessage(!emailValid ? "Invalid Email!" : "");
            setPasswordErrorMessage(!passwordValid ? "Password has to be atleast 6 characters long!" : "");
            setRepeatedPasswordErrorMessage(!repeatedPasswordValid ? "Passwords have to match!" : "");
        } else {
    ...
    return (
        <div className="register">
            ...
            <div className="register__container">
                <CustomInput names={["First Name", "Last Name"]} parentCallback={(data) => setNameObject({...nameObject, ...data})}/>
                <p className="register__error">{nameErrorMessage}</p>

                <CustomInput names={"Email"} parentCallback={(data) => setEmail(data["Email"])}/>
                <p className="register__error">{emailErrorMessage}</p>

                <CustomInput names={"Password"} parentCallback={(data) => setPassword(data["Password"])}/>
                <p className="register__error">{passwordErrorMessage}</p>

                <CustomInput names={"Repeat Password"} parentCallback={(data) => setRepeatedPassword(data["Repeat Password"])}/>
